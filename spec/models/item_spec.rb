...
RSpec.describe Item, type: :model do
  describe '#create' do
    ...
    it "genre_idが１より大きければ登録できる" do
      @item.genre_id = 3
      expect(@item).to be_valid
    end
    it "condition_idが１より大きければ登録できる" do
      @item.condition_id = 3
      expect(@item).to be_valid
    end
    it "delivery_fee_idが１より大きければ登録できる" do
      @item.delivery_fee_id = 3
      expect(@item).to be_valid
    end
    it "prefecture_idが１より大きければ登録できる" do
      @item.prefecture_id = 3
      expect(@item).to be_valid
    end
    it "day_idが１より大きければ登録できる" do
      @item.day_id = 3
      expect(@item).to be_valid
    end
  end
